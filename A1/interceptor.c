...
/* Note to self: Why would a list be needed ?
    Likely because there could multiple calls made to the syscall were are meant to intercept.
    And so we need a way to track all the processes that made said syscall. A list
...
{
	// Delete current->pid from all lists
	del_pid(current->pid);
    
	// Call the original exit_group function with arg status
...
/**
 * This is the de-intercept function.
 * It iterators through my_table of syscalls and de-intercepts (resets to original state)
 * all system calls that were being monitored and clears any pids tied to such syscalls.
 *
 */

static int deinterceptor(void){
    
    int i = 0;
    
	for(i = 1; i < NR_syscalls; i++){
		if(table[i].intercepted){
			sys_call_table[i] = table[i].f; // Reset to original sys call
			if(table[i].monitored > 0)
                destroy_list(i); // Remove pids associated with monitored syscall
		}
	}
...
asmlinkage long interceptor(struct pt_regs reg) {
    ...
    // Syscall is not being monitored for any pid so don't execute log
    if(!(table[syscall].monitored)){
        spin_unlock(&my_table_lock);
        return table[syscall].f(reg);
    }
    
    // Syscall is being monitored for all pids so execute log
    ...
    // If syscall is monitored for the current pid execute log, otherwise don't
	if(table[syscall].monitored && check_pid_monitored(syscall, current->pid)){
...
/*
Helper function, checks for valid syscall
*/
static int check_valid_syscall(int syscall){
...
/*
Helper function. checks for valid pid
*/
static int check_valid_pid(int pid){
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
    ...
    // Check to see if cmd is either of the acceptable ones:
    // request intercept (1), request release (2), start monitoring (3), stop monitoring (4)
    if(cmd > 4 || cmd < 1)
        return -EINVAL;
	...
	if (cmd == REQUEST_SYSCALL_INTERCEPT){
		...
		//check that syscall is already being intercepted
		...
		// Next stage, acquire lock for sys_call_table
        spin_lock(&sys_call_table_lock);
        
        // Save original sys_call
		...
		//check if syscall has not been intercepted yet
		...
		//check if current is root, if not check if current process requested pid
		if (current_uid() != 0){
		...
		//blacklist case, 'all' pids being monitored
		if (table[syscall].monitored == 2){
			//check if pid is on the blacklist
			if (check_pid_monitored(syscall,pid) == 0){
				// pid is not on blacklist, therefore it is already being monitored
				spin_unlock(&my_table_lock);
				return -EBUSY;
			}
		...
		//some pids being monitored case
			...
			//if request to monitor all pids
			else if (pid == 0){ 
				destroy_list(syscall);
		...
		//check if current process is root, if not check if current process requested pid
		if (current_uid() != 0){
			if (pid == 0 || check_pids_same_owner(current->pid,pid) != 0){
                return -EPERM;
			}
		...
		//if monitoring no pids
		if (table[syscall].monitored == 0){
			spin_unlock(&my_table_lock);
			return -EINVAL;
		}

		//monitoring some pids case
		else if (table[syscall].monitored == 1){
			if (check_pid_monitored(syscall,pid) == 0){
				spin_unlock(&my_table_lock);
				return -EINVAL;
				...
				del_pid_sysc(pid,syscall);
				//if pid list is empty set monitored to 0
			}
		}

		//blacklist case
		else if (table[syscall].monitored == 2){
			//if request to stop monitoring all pids
			if (pid == 0){
				destroy_list(syscall);
			}
			//check if already on the blacklist
...
static int init_function(void) {
    ...
    spin_lock(&my_table_lock);
    // Initialize lists for all my_table pid lists
    for(i = 1; i < NR_syscalls; i++){
	...
	// Make sys_call_table readable/writable
	set_addr_rw((unsigned long)sys_call_table);

	// Save original my_custom_syscall and hijack it with ours
	orig_custom_syscall = sys_call_table[MY_CUSTOM_SYSCALL];
	sys_call_table[MY_CUSTOM_SYSCALL] = my_syscall;

	// Save original exit_group syscall and hijack it with ours
	orig_exit_group = sys_call_table[__NR_exit_group];
	sys_call_table[__NR_exit_group] = my_exit_group;
    
	// Reset sys_call_table to read only
	set_addr_ro((unsigned long)sys_call_table);
...
static void exit_function(void)
{
    ...
    // Reset original custom and exit syscalls
	sys_call_table[MY_CUSTOM_SYSCALL] = orig_custom_syscall;
	sys_call_table[__NR_exit_group] = orig_exit_group;
    
	// Deintercept all relevant syscalls and cleanup associated pid lists
	// Caution: Should deinterceptor make it's own call to set_addr_rw?
	deinterceptor();
